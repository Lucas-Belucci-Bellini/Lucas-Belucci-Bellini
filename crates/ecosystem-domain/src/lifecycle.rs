...
/// Repositórios que o `status_for()` marca "em desenvolvimento" pelo nome.
pub const ALWAYS_IN_DEVELOPMENT: [&str; 3] =
    ["Projeto-Baluarte", "Ark-Initiative", "CHIPS-Digital-Logic-Sim-Lucas-Belucci"];
/// Os domínios extraídos do Baluarte (`baluarte-*`) estão em backlog…
pub const BACKLOG_PREFIX: &str = "baluarte-";
/// …exceto este.
pub const BACKLOG_EXCEPTION: &str = "baluarte-obra-segura";
...
pub fn status_py_v1(repo: &RepoFacts, category: &str, now: DateTime<Utc>) -> &'static str {
    ...
    if repo.name.starts_with(BACKLOG_PREFIX) && repo.name != BACKLOG_EXCEPTION {
        return "🟡 In Development";
    }
